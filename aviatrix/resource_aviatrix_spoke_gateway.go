...
func resourceAviatrixSpokeGatewayCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if insaneMode {
		...
		if !goaviatrix.IsCloudType(gateway.CloudType, goaviatrix.AWSRelatedCloudTypes|goaviatrix.GCPRelatedCloudTypes|
			...
			return fmt.Errorf("insane_mode is only supported for AWS (1), GCP (4), Azure (8), OCI (16), AzureGov (32), AWSGov (256), AWS China (1024), AWS Top Secret (16384) and AWS Secret (32768)")
		}
	...
	if enableBgp {
		...
		return fmt.Errorf("enable_spoke_preserve_as_path only work for the BGP spoke gateway")
	}
...
func resourceAviatrixSpokeGatewayUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("enable_spoke_preserve_as_path") {
		...
		if enableSpokePreserveAsPath && !enableBgp {
			return fmt.Errorf("enable_spoke_preserve_as_path is not supported for Non-BGP Spoke during Spoke Gateway update")
		}
		if !enableSpokePreserveAsPath {
