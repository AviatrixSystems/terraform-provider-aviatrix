...
func TestAccAviatrixEdgeNEO_basic(t *testing.T) {
	if os.Getenv("SKIP_EDGE_NEO") == "yes" {
		t.Skip("Skipping Edge NEO test as SKIP_EDGE_NEO is set")
	}

	resourceName := "aviatrix_edge_neo.test"
...
func testAccCheckEdgeNEOExists(resourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if !ok {
			return fmt.Errorf("edge neo not found: %s", resourceName)
		}
		if rs.Primary.ID == "" {
			return fmt.Errorf("no edge neo id is set")
		}
		...
		if edgeSpoke.GwName != rs.Primary.ID {
			return fmt.Errorf("could not find edge neo")
		}
		return nil
	}
}

func testAccCheckEdgeNEODestroy(s *terraform.State) error {
	client := testAccProvider.Meta().(*goaviatrix.Client)

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "aviatrix_edge_neo" {
			continue
		}

		_, err := client.GetEdgeNEO(context.Background(), rs.Primary.Attributes["gw_name"])
		if err != goaviatrix.ErrNotFound {
			return fmt.Errorf("edge neo still exists")
		}
	}
