...
func resourceAviatrixTransitGatewayCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if insaneMode {
		if cloudType != goaviatrix.AWS && cloudType != goaviatrix.AZURE && cloudType != goaviatrix.GCP {
	...
	if enableEgressTransitFireNet && gateway.CloudType != goaviatrix.AWS && gateway.CloudType != goaviatrix.AZURE {
		return fmt.Errorf("'enable_egress_transit_firenet' is only supported in AWS and AZURE cloud providers")
	}
...
func resourceAviatrixTransitGatewayUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("enable_egress_transit_firenet") {
		enableEgressTransitFirenet := d.Get("enable_egress_transit_firenet").(bool)
		if enableEgressTransitFirenet && gateway.CloudType != goaviatrix.AZURE && gateway.CloudType != goaviatrix.AWS {
			return fmt.Errorf("'enable_egress_transit_firenet' is currently only supported on AWS and AZURE cloud providers")
		}
		if enableEgressTransitFirenet {
