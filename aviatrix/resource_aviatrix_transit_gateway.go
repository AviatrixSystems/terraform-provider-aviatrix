...
func resourceAviatrixTransitGatewayCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if goaviatrix.IsCloudType(cloudType, goaviatrix.AWSRelatedCloudTypes|goaviatrix.GCPRelatedCloudTypes|goaviatrix.OCIRelatedCloudTypes|goaviatrix.AliyunRelatedCloudTypes) {
	...
	if goaviatrix.IsCloudType(cloudType, goaviatrix.AWSRelatedCloudTypes|goaviatrix.AzureArmRelatedCloudTypes|goaviatrix.OCIRelatedCloudTypes|goaviatrix.AliyunRelatedCloudTypes) {
	...
	if insaneMode {
		if !goaviatrix.IsCloudType(cloudType, goaviatrix.AWSRelatedCloudTypes|goaviatrix.GCPRelatedCloudTypes|goaviatrix.AzureArmRelatedCloudTypes) {
			return fmt.Errorf("insane_mode is only supported for AWS (1), GCP (4), AZURE (8), AWSGOV (256), AWSCHINA (1024) or AZURECHINA (2048)")
		}
		if goaviatrix.IsCloudType(cloudType, goaviatrix.AWSRelatedCloudTypes) {
			...
			if d.Get("ha_subnet").(string) != "" && d.Get("ha_insane_mode_az").(string) == "" {
				return fmt.Errorf("ha_insane_mode_az needed if insane_mode is enabled for AWS (1), AWSGOV (256) or AWSCHINA (1024) clouds and ha_subnet is set")
			}
	...
	if enableGatewayLoadBalancer && goaviatrix.IsCloudType(cloudType, goaviatrix.AWSRelatedCloudTypes^goaviatrix.AWSCHINA) {
		return fmt.Errorf("'enable_gateway_load_balancer' is only supported by AWS (1) and AWSGOV (256)")
	}
	...
	if enableTransitFireNet && goaviatrix.IsCloudType(cloudType, goaviatrix.AWSRelatedCloudTypes) {
...
func resourceAviatrixTransitGatewayRead(d *schema.ResourceData, meta interface{}) error {
	...
	if gw != nil {
		...
		if gwDetail.EnableTransitFireNet && goaviatrix.IsCloudType(gw.CloudType, goaviatrix.GCPRelatedCloudTypes) {
...
func resourceAviatrixTransitGatewayUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.Get("enable_egress_transit_firenet").(bool) && !goaviatrix.IsCloudType(gateway.CloudType, goaviatrix.AWSRelatedCloudTypes|goaviatrix.GCPRelatedCloudTypes|goaviatrix.AzureArmRelatedCloudTypes^goaviatrix.AZURECHINA) {
	...
	if d.HasChange("ha_subnet") || d.HasChange("ha_zone") || d.HasChange("ha_insane_mode_az") ||
		...
		deleteHaGw := false
		changeHaGw := false
		if goaviatrix.IsCloudType(transitGw.CloudType, goaviatrix.AWSRelatedCloudTypes|goaviatrix.AzureArmRelatedCloudTypes|goaviatrix.AliyunRelatedCloudTypes) {
...
func resourceAviatrixTransitGatewayDelete(d *schema.ResourceData, meta interface{}) error {
	...
	enableTransitFireNet := d.Get("enable_transit_firenet").(bool)
	if enableTransitFireNet && goaviatrix.IsCloudType(gateway.CloudType, goaviatrix.AWSRelatedCloudTypes) {
