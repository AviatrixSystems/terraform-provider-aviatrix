	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"eip_map": {
				Type:        schema.TypeMap,
				Optional:    true,
				Description: "EIP map for the edge transit gateway. ",
				Elem: &schema.Schema{
					Type: schema.TypeList,
					Elem: &schema.Resource{
						Schema: map[string]*schema.Schema{
			...
			"ha_eip_map": {
				Type:        schema.TypeMap,
				Optional:    true,
				Description: "EIP map for the HA edge transit gateway.",
				Elem: &schema.Schema{
					Type: schema.TypeList,
					Elem: &schema.Resource{
						Schema: map[string]*schema.Schema{
...
func resourceAviatrixTransitGatewayCreate(d *schema.ResourceData, meta interface{}) error {
	...
	// create edge transit gateway if the cloud type is AEP or Equinix
	if goaviatrix.IsCloudType(cloudType, goaviatrix.EdgeRelatedCloudTypes) {
		...
		if enable_edge_ha {
			...
			peerBackupPort, ok := d.GetOk("peer_backup_port")
			if !ok || peerBackupPort.(string) == "" {
				return fmt.Errorf("peer_backup_port is not set or not a valid string for edge transit gateway")
			}
			connectionType, ok := d.GetOk("connection_type")
			if !ok || connectionType.(string) == "" {
				return fmt.Errorf("connection_type is not set or not a valid string for edge transit gateway")
			}
			...
			// get the interfaces for the HA gateway
			interfaces, ok := d.Get("ha_interfaces").([]interface{})
			if !ok {
				return fmt.Errorf("ha_interfaces attribute is required for HA Edge Transit Gateway")
			}
			interfaceList, err := getInterfaceDetails(interfaces)
			if err != nil {
				return fmt.Errorf("failed to get ha_interface details: %s", err)
			}
			transitHaGw.Interfaces = interfaceList
...
func resourceAviatrixTransitGatewayRead(d *schema.ResourceData, meta interface{}) error {
	...
	if goaviatrix.IsCloudType(gw.CloudType, goaviatrix.EdgeRelatedCloudTypes) {
		...
		if len(gw.HaGw.Interfaces) != 0 {
			haInterfaces := setInterfaceDetails(gw.HaGw.Interfaces)
...
func resourceAviatrixTransitGatewayUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if enableEdgeHa && d.HasChange("ha_interfaces") {
		interfaceList, ok := d.Get("ha_interfaces").([]interface{})
		if !ok {
			return fmt.Errorf("invalid interfaces for EAT HA gateway")
		}
		interfaces, err := getInterfaceDetails(interfaceList)
		if err != nil {
			return fmt.Errorf("failed to get ha_interface details: %s", err)
...
func resourceAviatrixTransitGatewayDelete(d *schema.ResourceData, meta interface{}) error {
	...
	haSubnet := d.Get("ha_subnet").(string)
	haZone := d.Get("ha_zone").(string)
	enableEdgeHa := d.Get("enable_edge_ha").(bool)

	if haSubnet != "" || haZone != "" || enableEdgeHa {
		gateway.GwName += "-hagw"
...
func getEipMapDetails(eipMapList map[string]interface{}) (string, error) {
	if (eipMapList == nil) || (len(eipMapList) == 0) {
		return "", nil
	}
	eipMap := make(map[string][]map[string]string)
	for key, value := range eipMapList {
...
func setEipMapDetails(eipMap []goaviatrix.EIPMap) []map[string]interface{} {
	eipMapList := make([]map[string]interface{}, 0)
	for _, eip := range eipMap {
		for _, ip := range eip.IPs {
			eipMapDict := make(map[string]interface{})
			eipMapDict["interface"] = eip.InterfaceName
			eipMapDict["private_ip"] = ip.PrivateIP
			eipMapDict["public_ip"] = ip.PublicIP
			eipMapList = append(eipMapList, eipMapDict)
		}
	}
	return eipMapList
}
