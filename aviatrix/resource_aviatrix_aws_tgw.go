	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"manage_security_domain": {
				...
				Optional: true,
				Default:  true,
				Description: "This parameter is a switch used to determine whether or not to manage security domains " +
					"to the TGW using the aviatrix_aws_tgw resource. If this is set to false, security domains must " +
					"be managed using the aviatrix_aws_tgw_security_domain resource. Valid values: true, false. Default value: true.",
			},
...
func resourceAviatrixAWSTgwCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if manageSecurityDomain {
		...
		if manageVpcAttachment {
			return fmt.Errorf("\"manage_vpc_attachment\" must be false if \"manage_security_domain\" is false, " +
				"please use \"aviatrix_aws_tgw_vpc_attachment\"")
		}

		if manageTransitGwAttachment {
			return fmt.Errorf("\"manage_transit_gateway_attachment\" must be false if \"manage_security_domain\" " +
				"is false, please use \"aviatrix_aws_tgw_transit_gateway_attachment\"")
		}

		if (len(d.Get("security_domains").([]interface{})) > 0) || (len(d.Get("attached_aviatrix_transit_gateway").([]interface{})) > 0) {
			return fmt.Errorf("\"security_domains\" and \"attached_aviatrix_transit_gateway\" must be empty " +
				"if \"manage_security_domain\" is false, please use \"aviatrix_aws_tgw_vpc_attachment\" and " +
...
func resourceAviatrixAWSTgwRead(d *schema.ResourceData, meta interface{}) error {
	...
	if tgwName == "" {
		...
		d.Set("tgw_name", id)
		d.Set("manage_security_domain", true)
	...
	if manageSecurityDomain {
		...
		d.Set("security_domains", nil)
		d.Set("attached_aviatrix_transit_gateway", nil)
	}
...
func resourceAviatrixAWSTgwUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	manageSecurityDomain := d.Get("manage_security_domain").(bool)
	if d.HasChange("manage_security_domain") && !manageSecurityDomain {
		if manageVpcAttachment {
			return fmt.Errorf("\"manage_vpc_attachment\" must be false if \"manage_security_domain\" is false, " +
				"please use \"aviatrix_aws_tgw_vpc_attachment\"")
		}
	...
	d.Set("manage_security_domain", manageSecurityDomain)

	if manageSecurityDomain {
		// special case: manage_security_domain is changed from false to true
		if d.HasChange("manage_security_domain") {
			// when manage_security_domain is false, aws_tgw doesn't have any security domain related information,
			// nothing will be returned from GetChange(), have to read DB
			...
			// handling attached gateways
			oldAGWList := awsTgw.AttachedAviatrixTransitGW

			_, newAGW := d.GetChange("attached_aviatrix_transit_gateway")
			if newAGW == nil {
			...
			// handling security domains, domain connection, attached vpc
			mapSecurityDomainsOld := make(map[string][3]bool)
			mapSecurityDomainsNew := make(map[string][3]bool)

			oldSD := awsTgw.SecurityDomains

			_, newSD := d.GetChange("security_domains")
			if newSD == nil {
				newSD = new([]interface{})
			}
			...
			for _, dn := range oldSD {
				domainsOld = append(domainsOld, dn.Name)

				mapSecurityDomainsOld[dn.Name] = [3]bool{dn.AviatrixFirewallDomain, dn.NativeEgressDomain, dn.NativeFirewallDomain}

				for _, connectedDomain := range dn.ConnectedDomain {
					tempDomainConn := []string{dn.Name, connectedDomain}
					domainConnOld = append(domainConnOld, tempDomainConn)
				}

				if dn.Name == "Aviatrix_Edge_Domain" && len(dn.AttachedVPCs) != 0 {
					return fmt.Errorf("validation of source file failed: no VPCs should be attached to 'Aviatrix_Edge_Domain'")
				}
				for _, vpcSolo := range dn.AttachedVPCs {
					tempAttachedVPC := []string{dn.Name, vpcSolo.VpcID, vpcSolo.AccountName, vpcSolo.Region}
					...
					if vpcSolo.CustomizedRoutes != "" {
						tempAttachedVPC = append(tempAttachedVPC, vpcSolo.CustomizedRoutes)
					} else {
						tempAttachedVPC = append(tempAttachedVPC, "")
					}

					if vpcSolo.CustomizedRouteAdvertisement != "" {
						tempAttachedVPC = append(tempAttachedVPC, vpcSolo.CustomizedRouteAdvertisement)
					} else {
						tempAttachedVPC = append(tempAttachedVPC, "")
					}

					if vpcSolo.Subnets != "" {
						tempAttachedVPC = append(tempAttachedVPC, vpcSolo.Subnets)
					} else {
						tempAttachedVPC = append(tempAttachedVPC, "")
					}

					if vpcSolo.RouteTables != "" {
						tempAttachedVPC = append(tempAttachedVPC, vpcSolo.RouteTables)
					} else {
						tempAttachedVPC = append(tempAttachedVPC, "")
					}
