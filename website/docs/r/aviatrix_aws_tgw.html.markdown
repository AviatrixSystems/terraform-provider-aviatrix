...
* `attached_vpc` - (Optional) A list of VPCs attached to the domain (name: `security_domain_name`) together with its creation. This list needs to be null for "Aviatrix_Edge_Domain".
  ...
  * `disable_local_route_propagation` - (Optional) Advanced option. Switch to disable Spoke VPC CIDR propagation to the security domain/TGW route table that it is being attached to. Valid values: true, false. Default value: false.
