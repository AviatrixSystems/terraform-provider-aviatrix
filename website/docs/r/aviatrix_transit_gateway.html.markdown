...
The AviatrixTransitGateway resource manages the Aviatrix Transit Network Gateways
...
* `enable_nat` - (Optional) Enable NAT for this container. (Supported values: true of false)
...
* `connected_transit` - (Optional) Specify Connected Transit status. (Supported values: true or false)
