...
func (c *Client) ConnectTransitGwToTGW(gateway *TransitVpc, tgwName string, domainName string) error {
	Url, err := url.Parse(c.baseURL)
	if err != nil {
	...
	connectFireNetWithTgw.Add("vpc_id", gateway.VpcID)
	connectFireNetWithTgw.Add("tgw_name", tgwName)
	connectFireNetWithTgw.Add("domain_name", domainName)

	Url.RawQuery = connectFireNetWithTgw.Encode()
	resp, err := c.Get(Url.String(), nil)
	if err != nil {
		return errors.New("HTTP Post connect_firenet_with_tgw failed: " + err.Error())
	}
	var data APIResp
	if err = json.NewDecoder(resp.Body).Decode(&data); err != nil {
	...
	if !data.Return {
		return errors.New("Rest API connect_firenet_with_tgw Post failed: " + data.Reason)
	}
	return nil
}

func (c *Client) DisconnectTransitGwFromTGW(gateway *TransitVpc) error {
	Url, err := url.Parse(c.baseURL)
	if err != nil {
	...
	disconnectFireNetWithTgw.Add("vpc_id", gateway.VpcID)

	Url.RawQuery = disconnectFireNetWithTgw.Encode()
	resp, err := c.Get(Url.String(), nil)
	if err != nil {
		return errors.New("HTTP Post disconnect_firenet_with_tgw failed: " + err.Error())
	}
	var data APIResp
	if err = json.NewDecoder(resp.Body).Decode(&data); err != nil {
	...
	if !data.Return {
		return errors.New("Rest API disconnect_firenet_with_tgw Post failed: " + data.Reason)
	}
	return nil
}
