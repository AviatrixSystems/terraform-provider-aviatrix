...
def generate_vendor_json(
    github_path, vendor_json, origin_terr="", site_name="github.com"
):
    ...
    for dir_ in dir_list:
        for subdir_ in get_immediate_subdirectories(os.path.join(github_path, dir_)):
            ...
            vendor_json["package"].append(
                {
                    "checksumSHA1": check_sum,
                    "origin": origin,
                    "path": path,
                    "revision": commit,
...
def main():
    ...
    delete_everything_except(
        os.path.join(*[gopath, "src"]), exception_name="github.com"
    )
    delete_everything_except(
        os.path.join(*[gopath, "src", "github.com"]),
        exception_name="terraform-providers",
    )
    delete_everything_except(
        os.path.join(*[terraform_path, "vendor"]), exception_name="github.com"
    )
    delete_everything_except(
        os.path.join(*[terraform_path, "vendor", "github.com"]),
    ...
    move_everything_except(
        os.path.join(*[gopath, "src"]),
        exception_name=["github.com"],
    ...
    print("Dependencies\n-----------")
    generate_vendor_json(
        os.path.join(*[terraform_path, "vendor", "github.com"]), vendor_json
    )
    hashicorp_path = os.path.join(
        *[terraform_path, "vendor", "github.com", "hashicorp", "terraform", "vendor"]
    )
    for dir_ in get_immediate_subdirectories(hashicorp_path):
        generate_vendor_json(
            os.path.join(hashicorp_path, dir_),
            vendor_json,
    ...
    move_everything_except(
        os.path.join(hashicorp_path, "github.com"),
        exception_name=["google", "hashicorp", "pkg"],
        dest_dir=os.path.join(*[terraform_path, "vendor", "github.com"]),
    )
    move_everything_except(
        os.path.join(hashicorp_path, "github.com", "google"),
        exception_name=["go-querystring"],
        dest_dir=os.path.join(*[terraform_path, "vendor", "github.com", "google"]),
    )
    move_everything_except(
        os.path.join(hashicorp_path, "github.com", "hashicorp"),
        exception_name=["****"],
        dest_dir=os.path.join(*[terraform_path, "vendor", "github.com", "hashicorp"]),
    )
