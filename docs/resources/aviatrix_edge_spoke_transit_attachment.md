...
* `insane_mode_tunnel_number` - (Optional) Insane mode tunnel number, requires an integer value. Valid range for HPE over private network: 0-49. Valid range for HPE over internet: 2-20. A value of `0` uses the maximum number of tunnels for the gateway's instance size.
