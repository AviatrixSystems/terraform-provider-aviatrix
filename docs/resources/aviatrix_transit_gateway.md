...
* `ha_ipv6_subnet` - (Optional) The IPv6 CIDR block of the subnet used to create the HA Transit Gateway. This argument is supported only on AWS, Azure, AzureGov, and AWSGov. It must be specified when `enable_ipv6` is set to true and HA is enabled.
...
* `subnet_ipv6_cidr` - (Optional) The IPv6 CIDR block of the subnet used to create the Transit Gateway. This argument is supported only on AWS, Azure, AzureGov, and AWSGov. It must be specified when `enable_ipv6` is set to true.
