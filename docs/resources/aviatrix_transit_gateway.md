...
* `ha_azure_eip_name` - (Optional) Name of public IP Address resource in Azure to be assigned to the HA Transit Gateway instance. Required if `ha_eip` is set and `cloud_type` is Azure, AzureGov or AzureChina. Available as of provider version 2.20+.
...
* `azure_eip_name` - (Optional) Name of public IP Address resource in Azure to be assigned to the Transit Gateway instance. Required if `allocate_new_eip` is false and `cloud_type` is Azure, AzureGov or AzureChina. Available as of provider version 2.20+.
