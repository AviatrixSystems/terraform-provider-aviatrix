...
resource "aviatrix_transit_gateway" "test_transit_gateway_gcp" {
  ...
  gw_name      = "avtxgw-gcp"
  vpc_id       = "vpc-gcp-test~-~project-name"
  vpc_reg      = "us-west2-a"
...
* `rx_queue_size` - (Optional) Gateway ethernet interface RX queue size. Once set, can't delete or disable. Available for AWS as of provider version R2.22+.
