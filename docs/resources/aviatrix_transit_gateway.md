...
* `eip` - (Optional) Required when `allocate_new_eip` is false and Private Mode is not enabled on the Controller. It uses the specified EIP for this gateway. Available in Controller version 4.7+. Only available for AWS, GCP, Azure, OCI, AzureGov, AWSGov, AWSChina, AzureChina, AWS Top Secret and AWS Secret.
