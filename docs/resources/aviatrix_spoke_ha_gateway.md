...
* `extended_zone` - (Optional) Extended Zone. Only available for Azure / Azure GOV / Azure CHINA gateway.
...
* `gw_size` - (Optional) Size of the Spoke HA Gateway instance. If not set, the HA gateway follows the primary gateway's size, including when the primary is later resized. If set, the specified size is used. Example: AWS/AWSGov/AWSChina: "t2.large", Azure/AzureGov/AzureChina: "Standard_B1s", OCI: "VM.Standard2.2", GCP: "n1-standard-1".
