...
| GitHub 2482 | Fixed a perpetual `terraform plan` diff (`+ number_of_retries = 0`, `+ retry_interval = 300`) on imported `aviatrix_edge_spoke_external_device_conn` resources. Both attributes are provider-side create-retry knobs that the Controller does not store, so the import now seeds them with their schema defaults instead of leaving them out of the state. |
